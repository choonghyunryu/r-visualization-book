...
getStock <- function(ticker="AAPL", startDate="2011-01-01", endDate=Sys.Date()) {
  preMonth <- function(x) {
    year <- substr(x, 1, 4)
    month <- sprintf("%02d", as.integer(substr(x, 6, 7))-1)
    day <- substr(x, 9, 10)
    
    paste(year, month, day, sep="-")
  }
  
  startDate <- preMonth(startDate)
  endDate <- preMonth(endDate)
  url <- paste("http://ichart.finance.yahoo.com/",
               "table.csv?s=", ticker,
...
Version <- installed.packages()["maps", "Version"]
...
if (Version < "3.1.0") {
  ...
  pos <- paste(pos, "mapdata", sep="/")
  map.data <- list.files(pos)
  map.data <- sapply(map.data, function(x) strsplit(x, "\\.")[[1]][1])
  map.data <- unique(map.data)
}
...
plotShp <- function (x, minArea=50000000, col=NULL, border = par("fg"), add = FALSE, xlim = NULL, 
                     ylim = NULL, xpd = NULL, density = NULL, angle = 45, pbg = NULL, 
                     axes = FALSE, lty = par("lty"), ...) 
{
  poly_i <- slot(x, "polygons")
  p_i <- slot(x, "plotOrder")
  ...
  for (i in p_i) {
    p_j <- slot(poly_i[[i]], "plotOrder")
    poly_j <- slot(poly_i[[i]], "Polygons")
    
    regions <- matrix(ncol=2)
    
    for(j in p_j) {
      if (slot(poly_j[[j]], "area") < minArea)
        next
      regions <- rbind(regions, coordinates(x@polygons[[i]]@Polygons[[j]]), NAr)
    }
    regions <- regions[-1, ]
    regions <- regions[-NROW(regions), ]
    
    if (!is.null(density)) 
      polypath(x = regions[, 1], y = regions[, 2], border = border[i], 
               col = col[i], xpd = xpd, density = density[j], angle = angle[j], lty = lty, bg = pbg, ...)
    else 
      polypath(x = regions[, 1], y = regions[, 2], border = border[i], 
               col = col[i], lty = lty, xpd = xpd, bg = pbg, ...)
  }
}  
...
## Dropbox API에서 source_DropboxData 함수를 더이상 지원하지 않음
# library(repmis)
# traffic <- source_DropboxData("traffic.csv", "wsjmfxx1cex5mbv", sep=",", header=TRUE)

url <- "https://dl.dropboxusercontent.com/u/46305178/datas/traffic.csv"
traffic <- read.csv(url, row.names=NULL)
...
# ==================== 소스순번: 055 ==================== 
## 에러: geom_hex() only works with Cartesian coordinates라는 에러가 발생하여,
## 명시적으로 cartesian 좌표계로 변경하여 오류 수정
map <- qmap(location='서울', zoom=11, maptype="roadmap", legend = "topleft")
#p <- map + geom_hex(aes(x=start.pos.x, y=start.pos.y), bins=12,
#       alpha=0.6, color="white", data=traffic)
p <- map + coord_cartesian() +
  geom_hex(aes(x=start.pos.x, y=start.pos.y), bins=12,
           alpha=0.6, color="white", data=traffic)
