...
# ggplot2 2.0.0 이상에서는 movies 데이터프레임이 빠지고, ggplot2movies 패키지로 이동함
major <- strsplit((installed.packages()["ggplot2", "Version"]), "\\.")[[1]][1]
if (as.numeric(major) > 1) {
  if (!"ggplot2movies" %in% installed.packages()[, 1])
